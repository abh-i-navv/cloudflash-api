...
func CreateCollection(c domain.Collection) error {
	query := `INSERT INTO collections (id, name, created_at, updated_at)
	VALUES (?,?,?,?)`
...
func SaveRequest(r domain.SavedRequest) error {
	query := `
		INSERT INTO saved_requests(id, collection_id, folder_id, name,method, url, body, headers, params, created_at, updated_at)
		VALUES (?,?,?,?,?,?,?,?,?,?,?)
	`
	_, err := DB.Exec(query, r.ID, r.CollectionID, r.FolderID, r.Name, r.Method, r.URL, r.Body, r.Headers, r.Params, r.CreatedAt, r.UpdatedAt)
